    ...
    <div class="filters__area">
      <div class="filters__group">
        ...
        <label for="">
          ...
          <select v-if="filterKey==='role'" v-model="filterValue" name="filter">
            <option selected value="">select</option>
    ...
    <div class="home__content">
      <div class="table__container">
        <table>
          ...
          <tbody>
            <tr v-for="user in filteredData" :key="user.id" class="margin-botton">
              <td class="highlight">{{ user?.datos_personales?.nombres }} {{ user?.datos_personales?.apellidos }}</td>
              <td>{{ user.email }}</td>
              <td>{{ user.cargo }}</td>
              <td><span class="usuario--activo"> {{ user.estado }} </span></td>
...
const fetchData = () => {
  ...
  const { data: usersData } = useAsyncData<User[]>('users', () => $fetch(`${baseURL}/users?${query.value}`, {
    credentials: 'include',
    headers: {
  ...
  watch(usersData, () => {
    ...
    filteredData.value = usersData.value
  })
}
...
const applyFilter = () => {
  ...
  if (filterKey.value === 'nombre') {
    filteredData.value = users.value.filter((item) => {
      return item.datos_personales?.nombres + item.datos_personales?.apellidos === filterValue.value
    });
    
  } else if (filterKey.value === 'email') {
    filteredData.value = users.value.filter((item) => {
      return item.email === filterValue.value
    });
    
  } else if (filterKey.value === 'cedula') {
    filteredData.value = users.value.filter((item) => {
      return item.datos_personales?.cedula === filterValue.value
    });
    
  } else if (filterKey.value === 'role') {
    filteredData.value = users.value.filter((item) => {
      return item.role.name === filterValue.value
    });
...
const { data: totalData } = useAsyncData<number>('totalData', () => $fetch(`${baseURL}/users/count`, {
  credentials: 'include',
  headers: {
    'Content-Type': 'application/json',
    'Authorization': `Bearer ${authStore.token}`
...
const goToCreate = async () => {
  await router.push('/usuarios/crear')
}

const goToEdit = async (id: string) => {
  await router.push('/usuarios/' + id)
}

const deleteUser = async (user: User) => {
const uid = user.datos_personales?.id
if (uid) {
await $fetch(`${baseURL}/datos-personales/${uid}`, {
method: 'DELETE',
headers: {
'Content-Type': 'application/json',
'Authorization': 'Bearer ' + authStore.token
...
users.value?.splice(users.value.findIndex((val) => val.id === user.id), 1)
})
.catch((err: any) => {
console.error(err)
})
